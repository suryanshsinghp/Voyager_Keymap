...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [0] = LAYOUT_voyager(
    AS_TOGG,        M_CTRL,         M_GUI,          KC_X,           M_ALT,          KC_NO,                                          KC_NO,          OSM(MOD_HYP),   KC_J,           KC_NO,          KC_NO,         CM_TOGG,     
    SH_TOGG,        KC_Q,           KC_G,           KC_F,           KC_P,           OSM(MOD_HYPR),                                  TD(DANCE_R2),   KC_L,           KC_U,           KC_Y,           KC_Z,          SH_TOGG, 
    M_SHFT,         KC_A,           KC_R,           KC_S,           KC_T,           MOD_TOGG,                                       TD(DANCE_R1),   KC_N,           KC_E,           KC_I,           KC_O,          KC_RSFT,  
    KC_NO,          KC_V,           KC_W,           KC_C,           KC_D,           SH_MON,                                         TD(DANCE_27),   KC_H,           KC_M,           KC_B,           KC_K,          QK_RBT, 
                                                    LT(1, KC_ESCAPE),KC_BSPC,                                       KC_ENTER,       LT(2,KC_SPACE)
  ),
  [1] = LAYOUT_voyager(
    KC_NO,          KC_NO,          TO(0),          KC_NO,          KC_NO,          KC_NO,                                          KC_F4,          KC_F5,          KC_F6,          KC_F7,          KC_F8,          KC_F9,          
    KC_NO,          KC_NO,          TD(DANCE_0),    TD(DANCE_1),    TD(DANCE_2),    M_ALT,                                          KC_F3,          TD(DANCE_11),   TD(DANCE_12),   TD(DANCE_13),   KC_NO,          KC_F10,          
    M_SHFT,         TD(DANCE_3),    TD(DANCE_4),    TD(DANCE_5),    TD(DANCE_6),    M_GUI,                                          KC_F2,          TD(DANCE_14),   TD(DANCE_15),   TD(DANCE_16),   TD(DANCE_17),   KC_F11,          
    KC_NO,          TD(DANCE_7),    TD(DANCE_8),    TD(DANCE_9),    TD(DANCE_10),   M_CTRL,                                         KC_F1,          TD(DANCE_18),   TD(DANCE_19),   TD(DANCE_20),   TD(DANCE_23),   KC_F12,          
                                                    TO(0),          KC_BSPC,                                        KC_BSPC,        TO(0)
  ),
  [2] = LAYOUT_voyager(
    KC_NO,          KC_NO,          TO(0),          KC_NO,          KC_NO,          KC_NO,                                          KC_BRIGHTNESS_DOWN,KC_BRIGHTNESS_UP,TD(DANCE_21),    KC_AUDIO_VOL_UP, KC_NO,          KC_NO, 
    ...
    KC_NO,          TD(DANCE_23),   KC_LEFT,        KC_DOWN,        KC_RIGHT,       KC_ESC,                                         KC_DELETE,         M_CTRL,          M_GUI,           M_ALT,           M_SHFT,         QK_RBT,
...
const uint16_t PROGMEM combo31[] = {KC_A, KC_R, COMBO_END};                 // 0 capsFFFF
...
combo_t key_combos[COMBO_COUNT] = {
    ...
    COMBO(combo1, KC_TAB),
    COMBO(combo2, TO(2)),
    COMBO(combo3, CLRMODS),
    COMBO(combo4, CLRMODS),
    COMBO(combo5, TO(1)),
    COMBO(combo6, KC_LEFT),
    COMBO(combo7, KC_RIGHT),
    COMBO(combo8, TD(DANCE_R2)),
    COMBO(combo9, M_SHFT),
    COMBO(combo10, M_ALT),
    COMBO(combo11, TD(DANCE_27)),
    ...
    COMBO(combo21, TD(DANCE_R1)),
    COMBO(combo22, KC_Q),
    COMBO(combo23, KC_Z),
    COMBO(combo24, M_SHFT),
    ...
    COMBO(combo33, M_GUI),
    COMBO(combo34, M_CTRL),
    COMBO(combo35, KC_DOWN),
    COMBO(combo36, M_GUI),
    COMBO(combo38, KC_DELETE),
    COMBO(combo40, M_ALT),
    COMBO(combo41, LLOCK),
...
void set_layer_color(int layer)
{
    ...
    { // layer 1 and 2
        ...
        if (timer_elapsed32(Mouse_Setting_timer) < 3000)
        {
            ...
            RGB rgb = hsv_to_rgb(hsv);
            float f = (float)rgb_matrix_config.hsv.v / UINT8_MAX;
            for (int i = 0; i < mk_wheel_max_speed; i++)
            { // mouse wheel speed
                rgb_matrix_set_color(i, f * rgb.r, f * rgb.g, f * rgb.b);
            }

            for (int i = 0; i < mk_max_speed; i++)
            { // mouse move speed
...
bool process_record_user(uint16_t keycode, keyrecord_t *record)
{
    ...
    case KC_MS_UP:
        ...
        {
            if (LLockInit)
            { // Only lock once-add timer as well-only layer lock if more than 2 sec
                LLockInit = false;
                LLockedOnce = true;
            }
        }
    ...
    case M_SHFT: // Toggles key
        ...
        { // When key is released.
            shft_trigger = false;
            if (timer_elapsed32(shft_timer) < ModQuickHoldTime)
            { // If timer is less than 200ms.
                if (held4)
                { // If shift is held
    ...
    case M_CTRL: // Toggles key
        ...
        { // When key is released.
            ctrl_trigger = false;
            if (timer_elapsed32(ctrl_timer) < ModQuickHoldTime)
            { // If timer is less than 200ms.
                if (held3)
                { // If shift is held
    ...
    case M_ALT: // Toggles key
        ...
        { // When key is released.
            alt_trigger = false;
            if (timer_elapsed32(alt_timer) < ModQuickHoldTime)
            { // If timer is less than 200ms.
                if (held5)
                { // If shift is held
    ...
    case M_GUI: // Toggles key
        ...
        { // When key is released.
            gui_trigger = false;
            if (timer_elapsed32(gui_timer) < ModQuickHoldTime)
            { // If timer is less than 200ms.
                if (held6)
                { // If shift is held
...
void dance_R1_finished(tap_dance_state_t *state, void *user_data)
{
    ...
    case SINGLE_TAP:
        ...
        { // impelement set_oneshot_mods(MOD_BIT(KC_RCTL)) for mods
...
void leader_start_user(void)
{
    ...
    if (OSdetectedOnce == false)
    { // remove this if post init user works
        if (detected_host_os() == OS_MACOS || detected_host_os() == OS_IOS)
        {
            onMac = true;
            onWin = false;
...
void leader_end_user(void)
{
    ...
    else if (leader_sequence_two_keys(KC_N, KC_J))
    ...
    else if (leader_sequence_three_keys(KC_N, KC_T, KC_P))
    { // new terminal in path
        mem_keycode_Leader[0] = KC_N;
        mem_keycode_Leader[1] = KC_T;
...
void matrix_scan_user(void)
{
    if (shft_trigger)
    {
        if ((!shft_held_once) & (timer_elapsed32(shft_timer) > ModQuickHoldTime))
        { // do these comp only if shft is pressed
